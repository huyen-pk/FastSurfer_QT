...
*   **Data IO Compliance:** Ensure absolute fidelity in reading and writing headers (e.g., the `OpenHC::imaging::mri::fastsurfer::MghImage` header parsing, RAS orientation matrices, and affine transforms) so downstream medical tools (like FreeSurfer) parse the output flawlessly.
...
| **PhilipPhilips MRI test data** | geometry validation & math correctness | triply oblique scans | Triply oblique scans with angles offset from each of these image planes | [specs](./specs/core/imaging/geometric_transformation_test_validation/oblique_geometry_test_plan.md) |
...
- Draft manifest: `_specs/{modules}/evaluation_data_manifest.csv`
...
- Prefer **ITK** for 3D image morphology, contour extraction, spacing-aware distance maps, and overlap-style image metrics because it is already linked in `core/CMakeLists.txt`.
