...
- Document the approach under `specs/` on the current branch.
- Scope the work to `core/tests/imaging/mri/fastsurfer/test_step_conform.cpp` probe-generation and probe-evaluation helpers.
- Do not change the native conform implementation in `core/src/imaging/mri/fastsurfer`; improve the regression test oracle only.
...
- The touched helper lives in `core/tests/imaging/mri/fastsurfer/test_step_conform.cpp`.
...
- The production interpolation path has explicit boundary handling in `core/src/imaging/mri/fastsurfer/step_conform.cpp`, so boundary coverage in the tests is important.
...
- deterministic probe-generation improvements in `core/tests/imaging/mri/fastsurfer/test_step_conform.cpp`,
- documentation of the testing strategy and rationale,
- focused validation using the rebuilt `test_step_conform` executable.

Out of scope:

- production conform-step code changes in `core/src/imaging/mri/fastsurfer/step_conform.cpp`,
...
- `core/tests/imaging/mri/fastsurfer/test_step_conform.cpp`

New documentation artifact:

- `specs/core/imaging/probe_generation_improvements/implementation_plan.md`

## Validation Plan

Focused validation should rebuild the `core/` test target and run the conform-step cases that exercise the helper:
...
The focused validation completed successfully after regenerating the stale `core/build` tree so it picked up the current `test_step_conform` target.
