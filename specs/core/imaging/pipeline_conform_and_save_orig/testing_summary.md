...
Verification for the native `core/` MGZ conform step and its desktop integration in `_app/desktop`.

## What Was Verified

### 1. Source Diagnostics

- VS Code diagnostics for the touched `core/` and `_app/desktop` paths reported no editor-detected errors after implementation.
...
- the test executables were produced in `core/build/tests`.
...
- `_app/desktop` linked against the new `core/` library,
...
- The first `core/` implementation is intentionally limited to the already-conformed MGZ path required by the approved Subject140 fixture.
