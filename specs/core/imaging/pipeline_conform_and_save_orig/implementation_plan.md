...
- Use `core/` for image-processing code and parity tests.
...
- execute the step through native C++ code in `core/`, and
...
- `core/` is the native processing module root.
...
- the runtime requirement remains native C++ in `core/`, with Python used only as the parity oracle.

This means MGZ/FreeSurfer-style image I/O is part of the first implementation scope rather than a later extension.

## Proposed Architecture

### 1. `core/` Native Processing Layer

Create a small C++ processing library under `core/` that owns the step behavior.
...
The implementation should use a native C++ imaging library from `core/` rather than wrapping the Python implementation.

Because the native processing module now lives in `core/`, the coding step should introduce or update the required native dependency and CMake wiring there.

The approval decision for implementation is therefore:

- proceed with an **MGZ-first native C++ conform implementation** in `core/`,
- keep Python only as the **parity oracle** in tests.

## Parity Test Strategy

Parity tests belong in `core/tests`.
...
- `core/` tests build and run successfully,
...
- isolate the dependency in `core/`,
- document the requirement in `core/` or `_app/desktop` build documentation,
...
- keep most behavioral verification in `core/` parity tests.

## Step 2 Approval Request

Implementation should proceed only if the following are approved:

- `core/` becomes the native home for the conform-and-save-original processing step.
