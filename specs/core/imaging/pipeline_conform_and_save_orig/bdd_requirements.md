...
Desktop-accessible native implementation of FastSurfer's `pipeline_conform_and_save_orig` step, split between `core/` for processing and `_app/desktop` for UI.
...
- `core/` contains the native image-processing implementation for this step.
...
- `core/` contains parity tests for the native implementation.
...
**Then** the desktop app should invoke the native `core/` processing service
...
**When** the native `core/` conform step runs on that fixture
...
- `core/` owns the real processing and test logic.
