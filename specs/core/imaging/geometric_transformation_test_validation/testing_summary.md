...
- `cmake --build core/build --target test_step_conform`
- `cd core/build/tests && ./test_step_conform oblique`
- `cd core/build/tests && ./test_nifti_converter`
...
- The workspace-level CTest tool resolved the wrong build tree in this environment, so the passing validation was executed directly from `core/build/tests`.
